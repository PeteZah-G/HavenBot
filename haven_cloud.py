...
async def on_message(message):
    ...
    if message.channel.id not in active_channels:
        ...
        if message.channel.id in pinned_messages and not message.content.startswith('h!'):
...
async def list_commands(interaction: discord.Interaction):
    embeds = []
    embed1 = discord.Embed(title="HavenBot Commands (1/2)", color=discord.Color.blue())
    embed1.add_field(name="h!initiate", value="Activates AI chat in the channel (Admin only).", inline=False)
    embed1.add_field(name="h!stop", value="Disables AI chat in the channel (Admin only).", inline=False)
    embed1.add_field(name="h!ban @user [reason]", value="Bans a user (Admin only).", inline=False)
    embed1.add_field(name="h!unban user_id [reason]", value="Unbans a user by ID (Admin only).", inline=False)
    embed1.add_field(name="h!kick @user [reason]", value="Kicks a user (Admin/Mod).", inline=False)
    embed1.add_field(name="h!mute @user [reason]", value="Mutes a user (Admin/Mod).", inline=False)
    embed1.add_field(name="h!unmute @user [reason]", value="Unmutes a user (Admin only).", inline=False)
    embed1.add_field(name="h!purge amount", value="Deletes up to 100 messages (Admin/Mod).", inline=False)
    embed1.add_field(name="h!lock [reason]", value="Locks the channel (Admin only).", inline=False)
    embed1.add_field(name="h!unlock [reason]", value="Unlocks the channel (Admin only).", inline=False)
    embed1.add_field(name="h!ping", value="Shows bot latency.", inline=False)
    embed1.add_field(name="h!userinfo [@user]", value="Shows user info (defaults to self).", inline=False)
    embed1.add_field(name="h!serverinfo", value="Shows server info.", inline=False)
    embed1.add_field(name="h!clearwarnings @user", value="Clears warnings for a user (Admin/Mod).", inline=False)
    embed1.add_field(name="h!warn @user [reason]", value="Warns a user (Admin/Mod).", inline=False)
    embeds.append(embed1)

    embed2 = discord.Embed(title="HavenBot Commands (2/2)", color=discord.Color.blue())
    embed2.add_field(name="h!warns [@user]", value="Shows warnings for a user (defaults to self).", inline=False)
    embed2.add_field(name="h!role add/remove @user @role", value="Adds or removes a role (Admin only).", inline=False)
    embed2.add_field(name="h!poll question option1 option2...", value="Creates a poll with up to 10 options.", inline=False)
    embed2.add_field(name="h!avatar [@user]", value="Shows user avatar (defaults to self).", inline=False)
    embed2.add_field(name="h!slowmode seconds", value="Sets channel slowmode (Admin only).", inline=False)
    embed2.add_field(name="h!invite", value="Creates a server invite link.", inline=False)
    embed2.add_field(name="h!afk [reason]", value="Sets AFK status with optional reason.", inline=False)
    embed2.add_field(name="h!afkstop", value="Removes AFK status.", inline=False)
    embed2.add_field(name="h!generateimage prompt", value="Generates an image from a prompt.", inline=False)
    embed2.add_field(name="h!nickname @user [nick]", value="Sets or clears a user's nickname (Admin only).", inline=False)
    embed2.add_field(name="h!roleinfo @role", value="Shows role info.", inline=False)
    embed2.add_field(name="h!pin message", value="Sets a message to be posted after every message in the channel.", inline=False)
    embed2.add_field(name="h!unpin", value="Removes the pinned message from the channel.", inline=False)
    embed2.add_field(name="h!say message", value="Sends a message as the bot (Admin only).", inline=False)
    embed2.add_field(name="h!embed message", value="Sends an embedded message (Admin only).", inline=False)
    embed2.add_field(name="h!reactionrole message_id @role emoji", value="Sets a reaction role (Admin only).", inline=False)
...
async def enable_mod_perms(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"Moderator permissions granted to {role.mention}. Members with this role can now use h!kick, h!mute, h!purge, and h!warn.", ephemeral=False)
